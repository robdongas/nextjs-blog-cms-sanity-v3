/**
 * This code is responsible for revalidating the cache when a project is updated.
 *
 ...
 * 6. Filter: _type == "project" || _type == "settings"
 * 7. Projection: Leave empty
 * 8. Status: Enable webhook
 * 9. HTTP method: PROJECT
...
async function parseBody<Body = SanityDocument>(
  ...
  if (req.readableEnded) {
    throw new Error(
      `Request already ended and the PROJECT body can't be read. Have you setup \`export {config} from 'next-sanity/webhook' in your webhook API handler?\``,
    )
  }
...
async function queryStaleRoutes(
  ...
  if (body._type === 'project') {
    ...
    if (!exists) {
      ...
      // If there's less than 3 projects with a newer date, we need to revalidate everything
      if (moreStories < 3) {
...
async function mergeWithMoreStories(
  ...
  const moreStories = await client.fetch(
    groq`*[_type == "project"] | order(date desc, _updatedAt desc) [0...3].slug.current`,
  )
